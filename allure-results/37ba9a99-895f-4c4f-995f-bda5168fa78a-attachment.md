...
Call log:
  - waiting for locator('//button[contains(text(),"Add Additional Information")]')
    - locator resolved to <button type="button" _ngcontent-hua-c367="" class="tw-flex tw-items-center tw-text-blue tw-text-bt-1 tw-font-medium tw-gap-2 focus:tw-outline-none">…</button>
  - attempting click action
    2 × waiting for element to be visible, enabled and stable
      - element is visible, enabled and stable
      - scrolling into view if needed
      - done scrolling
      - <in-payee-form _nghost-hua-c367="" _ngcontent-hua-c368="">…</in-payee-form> intercepts pointer events
...
```ts
  ...
  132 |         await this.page.locator('ng-dropdown-panel').waitFor({ state: 'hidden', timeout: 5000 });
> 133 |         await this.addAdditionalInfoButton.addButton.click();
      |                                                      ^ TimeoutError: locator.click: Timeout 15000ms exceeded.
  134 | 
  135 |         
  ...
  143 |         await this.addAdditionalInfoButton.stateArrow.click();
  144 |         await this.getStateOption(details.state).click();
  145 |         await this.addAdditionalInfoButton.postalCodeInput.fill(details.zip);
  146 |         await this.addAdditionalInfoButton.secondaryPhoneNumberInput.fill(details.secondaryPhone);
